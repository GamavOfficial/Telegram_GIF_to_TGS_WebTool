...
function deinterlace(src,w,h){
  const out=new Uint8Array(src.length), starts=[0,4,2,1], steps=[8,8,4,2];
  let s=0;
  for(let pass=0;pass<4;pass++)for(let y=starts[pass];y<h;y+=steps[pass]){
    out.set(src.subarray(s,s+w),y*w);s+=w;
  }
  return out;
}
function lzw(data,min,size){
  const clear=1<<min,end=clear+1;
  let codeSize=min+1,next=clear+2,bits=0,old=-1,first=0,op=0;
  ...
  function readCode(){
    ...
    for(let b=0;b<codeSize;b++){
      const k=bits>>3;if(k>=data.length)return null;
  ...
  while(op<size){
    ...
    if(next<4096){
      pre[next]=old;suf[next]=first;next++;
...
function parse(buffer){
  ...
  function skip(){while(p<n){const len=a[p++];if(!len)break;p+=len}}
  while(p<n){
    const b=a[p++];
    if(b===0x3b)break;
    if(b===0x21){
      ...
      if(label===0xf9){
        ...
        const ti=a[p++];p++;
        gce={delay:Math.max(20,delay||100),transparent:!!(flags&1),transIndex:ti,disposal:(flags>>2)&7};
        p=q+sz+1;
      }else skip();
      continue;
    }
    ...
    const comp=new Uint8Array(total);let cp=0;
    for(const c of chunks){comp.set(c,cp);cp+=c.length}
    let idx=lzw(comp,min,fw*fh);if(inter)idx=deinterlace(idx,fw,fh);
    const rgba=new Uint8ClampedArray(fw*fh*4);
    for(let i=0;i<fw*fh;i++){
      const c=table[idx[i]]||[0,0,0],al=(gce.transparent&&idx[i]===gce.transIndex)?0:255;
